...
/*   Updated: 2024/05/23 16:03:42 by gmastroc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	strolling_around(t_mlx_win *window, t_coord pos)
{
	...
	map[window->map.player.y][window->map.player.x] = FLOOR;
	map[window->map.exit.y][window->map.exit.x] = EXIT;
	map[pos.y][pos.x] = PLAYER;
	window->map.player = pos;

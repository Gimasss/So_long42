...
/*   Updated: 2024/05/22 15:47:02 by gmastroc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_lines(int rows, int cols, char **map_mat)//2
{
	...
	i = 0;
	if (rows <= 3 || cols <= 3) //per controllare anche mappa troppo grande
	{
		return (ft_printf("Map too small, wrong vibes\n", 2), 0); //error
	}
	while (i < rows)
	{
		int row_len = ft_strlen(map_mat[i]);
		if (cols != row_len)
		{
	...
	return 1; // Return 1 if all rows have the same length
}

int	check_walls(char **map_mat, int rows, int cols)//3 - i need to make double cycle for rows + cols
{
	int i;
	int j;
	int last_col;
...
int count_elements(int rows, int cols, char **map, char to_check)//4
{
	int i;
	int j;
	int count;
...
int	check_elements(int rows, int cols, t_mlx_win *game)//5
{
	char **map;
	int i;
	int j;
	...
	while (++i < rows)
	{
		...
		while (++j < cols) 
		{
			...
			&&  map[i][j] != '1' && map[i][j] != '0' && map[i][j] != 'N')
				return (0);
			if (map[i][j] == 'P')//i is rows is y - j is cols is x
				game->map.player = (t_coord){j, i}; 
			if (map[i][j] == 'E')//i is rows is y - j is cols is x

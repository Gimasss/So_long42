...
/*   Updated: 2024/05/22 20:12:12 by gmastroc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_sprites(t_mlx_win *game)//KEEP IN MAIN
{
	int	x;
	int	y;
	...
	game->spr.collectible = convert_image(game, "./images/collectible.xpm", x, y);
...
int gnl_countlines(char *map)
{
	...
	close(fd); // Close the file after counting the lines
	return(mat_len);
}

char	**get_map_matrix(char *map, int fd)//KEEP IN MAIN
{
	char **map_mat;
	char *map_line;
	int mat_len;
	...
	if (map_mat == NULL)
		return (NULL); // errore di allocazione da gestire
	fd = open(map, O_RDONLY); // Reopen the file to read the lines
	i = 0;
	map_line = get_next_line(fd);
	while (map_line != NULL)
	{
	...
	close(fd); // Close the file after reading the lines
	return (map_mat);
}
...
int	main(int argc, char *argv[])
{
	...
	fd_map = check_if_dir(&window, argc, argv);//mi ritorna n int/fd - devo assocarlo a una variabile per non perderlo
	//controll se mappa e' mappa
	...
	//ft_print_matrix(window.map.map_mat);
	map_checks(&window, window.size.x, window.size.y);
	window.mlx = mlx_init();
	window.mlx_win = mlx_new_window(window.mlx, window.size.x * IMG_WIDTH,
		window.size.y * IMG_HEIGHT, "It's Vaporwave");
	init_sprites(&window);
	render_and_place(&window);
	mlx_hook(window.mlx_win, DestroyNotify, StructureNotifyMask, ez_exit, &window);
	mlx_hook(window.mlx_win, KeyPress, KeyPressMask, &key_event, &window);//if any key is pressed call my fun key_event
	mlx_loop(window.mlx);
	return 0;
}
